                                            ...
                                            IHandle<ReplicationMessage.GetReplicationStats>
    {
        ...
        private bool ManageSubscriptions()
        {
            ...
            foreach (var subscription in _subscriptions.Values)
            {
                ...
                {
                    _publisher.Publish(new SystemMessage.VNodeConnectionLost(subscription.ReplicaEndPoint, subscription.ConnectionId));
                ...
                if (subscription.SendQueueSize >= MaxQueueSize
                    || subscription.LogPosition - Interlocked.Read(ref subscription.AckedLogPosition) >= ReplicaSendWindow)
                    continue;
        ...
        private class ReplicaSubscription: IDisposable
        {
            ...
            public Guid ConnectionId { get { return _connection.ConnectionId; } }
            public int SendQueueSize { get { return _connection.SendQueueSize; } }
            public bool IsConnectionClosed { get { return _connection.IsClosed; } }
            ...
            {
                return string.Format("Connection: {0:B}, ReplicaEndPoint: {1}, IsPromotable: {2}, RawSend: {3}, EOFSent: {4}, "
